...
gather_team_pages <- function(year, file_path){
  ...
  for (team in teams) {
    ...
    c <- (page %>%
            xml2::read_html() %>%
            rvest::html_nodes(".coach"))[[1]]

    links <- as.list(rvest::html_nodes(c, "a")[[1]])
    texts <- rvest::html_text(rvest::html_nodes(c, "a"))
    ...
    coaches <- coaches %>%
      ...
      dplyr::select("coach", "coach.link", "coach.link.ref")
        ...
        suppressWarnings(
          sched <- sched %>%
            ...
            dplyr::filter(.data$Location %in% c("Home", "Away", "Neutral",
                                                "Semi-Home","Semi-Away")) %>%
            dplyr::select(-"WinnerScore",-"LoserScore") %>%
